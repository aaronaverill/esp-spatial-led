 ...
 * @remarks 
 * This method returns the current FastLED CRGB structure as a bitmap (.bmp) file
 * Why bitmap?
 *  - It's a known format that is automatically handled by many clients such as web browsers
 *  - Unlike other formats there is no compression. This allows fast and efficient response to the request
 *  - The bitmap format allows 24 bit rgb data which can be memcpy() directly from the CRGB array
 *  - If a client needs raw RGB byte arrays they can easily pull it out starting at index = 54 (after the headers)
 * 
 */
void BmpFile::writeLedData(AsyncWebServerRequest *request,  const Services::ILedInfo* leds) {
